...
if not os.path.exists("tmp/"): os.system("mkdir -p tmp/")

def read_map_files(dates, hemi):
    for d in dates:
        year, month, day = d.year, d.month, d.day
        fname = ("/sd-data/{:d}/map/%s/{:d}{:02d}{:02d}.%s.cnvmap.s.bz2".format(year, year, month, day))%(hemi, hemi)
        tfname = ("tmp/{:d}{:02d}{:02d}.%s.cnvmap.s.bz2".format(year, month, day))%(hemi)
        if os.path.exists(fname):
            print("Fname - ", fname, tfname)
            try:
                os.system(" cp " + fname + " tmp/")
                os.system("bzip2 -d " + tfname)
                tfname = tfname.replace(".bz2", "")
                reader = pydarn.SuperDARNRead(tfname)
                _md = reader.read_dmap(tfname)
...
def create_hourly_averages(dates=[dt.datetime(2000,1,1), dt.datetime(2003,12,31)]):
